    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> FMTResult {     
        match &self {
            Command::Constant{ led_count, colour } => {
                writeln!(f, "Command::Constant -> ({}, {}, {}) x {}\r", colour.0, colour.1, colour.2, led_count)?
            },
            Command::Stream(slice) => {
                writeln!(f, "Command::Stream -> {:#x} for {}\r", slice.as_ptr() as u32, slice.len())?
            },
            Command::Pulse {start, end, led_count, frames, period} => {
